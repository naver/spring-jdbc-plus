...
public class BoardRepositoryTest {
					...
					.comments(Arrays.asList(
						...
						Comment.builder()
							...
							.audit(Audit.builder()
								.name("naver2")
								.memo(Memo.builder()
									.memo("board post1 comment2 audit memo")
									.build())
								.secret(AuditSecret.builder()
									.secret("secret2")
									.build())
					...
					.comments(Arrays.asList(
						Comment.builder()
							...
							.audit(Audit.builder()
								.name("naver3")
								.memo(Memo.builder()
									.memo("board post2 comment1 memo")
									.build())
								.secret(AuditSecret.builder()
									.secret("secret3")
									.build())
					...
					.comments(Arrays.asList(
						...
						Comment.builder()
							...
							.audit(Audit.builder()
								.name("naver5")
								.memo(Memo.builder()
									.memo("board2 comment2 audit memo")
									.build())
								.secret(null)
								.build())
							.build()
	...
	private void assertEquals(Board actual, Board target) {
		...
		if (actual.getAudit() == null) {
			...
			if (actual.getAudit().getMemo() == null) {
				...
				assertThat(actual.getAudit().getMemo().getMemo())
					.isEqualTo(target.getAudit().getMemo().getMemo());
			}

			if (actual.getAudit().getSecret() == null) {
				assertThat(target.getAudit().getSecret()).isNull();
			} else {
				assertThat(actual.getAudit().getSecret().getId())
					.isEqualTo(target.getAudit().getSecret().getId());
				assertThat(actual.getAudit().getSecret().getSecret())
					.isEqualTo(target.getAudit().getSecret().getSecret());
			}
		}
		...
		if (target.getConfigMap() == null || target.getConfigMap().isEmpty()) {
			...
			for (int i = 0; i < actualConfigs.size(); i++) {
				assertThat(actualConfigs.get(i).getKey())
					.isEqualTo(targetConfigs.get(i).getKey());
				assertThat(actualConfigs.get(i).getValue().getId())
					.isEqualTo(targetConfigs.get(i).getValue().getId());
				assertThat(actualConfigs.get(i).getValue().getConfigKey())
					.isEqualTo(targetConfigs.get(i).getValue().getConfigKey());
				assertThat(actualConfigs.get(i).getValue().getConfigValue())
					.isEqualTo(targetConfigs.get(i).getValue().getConfigValue());
			}
		}
	...
	private void assertEquals(Post actualPost, Post targetPost) {
		...
		if (actualPost.getMemo() == null) {
			...
			assertThat(actualPost.getMemo().getMemo())
				.isEqualTo(targetPost.getMemo().getMemo());
		}

		if (actualPost.getAudit() == null) {
			assertThat(targetPost.getAudit()).isNull();
		} else {
			assertThat(actualPost.getAudit().getId())
				.isEqualTo(targetPost.getAudit().getId());
			assertThat(actualPost.getAudit().getName())
				.isEqualTo(targetPost.getAudit().getName());

			if (actualPost.getAudit().getMemo() == null) {
				assertThat(targetPost.getAudit().getMemo()).isNull();
			} else {
				assertThat(actualPost.getAudit().getMemo().getMemo())
					.isEqualTo(targetPost.getAudit().getMemo().getMemo());
			}

			if (actualPost.getAudit().getSecret() == null) {
				assertThat(targetPost.getAudit().getSecret()).isNull();
			} else {
				assertThat(actualPost.getAudit().getSecret().getId())
					.isEqualTo(targetPost.getAudit().getSecret().getId());
				assertThat(actualPost.getAudit().getSecret().getSecret())
					.isEqualTo(targetPost.getAudit().getSecret().getSecret());
			}
		}
		...
		for (int i = 0; i < actualTags.size(); i++) {
			assertThat(actualComments.get(i).getId())
				.isEqualTo(postComments.get(i).getId());
			assertThat(actualComments.get(i).getContent())
				.isEqualTo(postComments.get(i).getContent());

			if (actualComments.get(i).getAudit() == null) {
				assertThat(postComments.get(i).getAudit()).isNull();
			} else {
				assertThat(actualComments.get(i).getAudit().getId())
					.isEqualTo(postComments.get(i).getAudit().getId());
				assertThat(actualComments.get(i).getAudit().getName())
					.isEqualTo(postComments.get(i).getAudit().getName());

				if (actualComments.get(i).getAudit().getMemo() == null) {
					assertThat(postComments.get(i).getAudit().getMemo()).isNull();
				} else {
					assertThat(actualComments.get(i).getAudit().getMemo().getMemo())
						.isEqualTo(postComments.get(i).getAudit().getMemo().getMemo());
				}

				if (actualComments.get(i).getAudit().getSecret() == null) {
					assertThat(postComments.get(i).getAudit().getSecret()).isNull();
				} else {
					assertThat(actualComments.get(i).getAudit().getSecret().getId())
						.isEqualTo(postComments.get(i).getAudit().getSecret().getId());
					assertThat(actualComments.get(i).getAudit().getSecret().getSecret())
						.isEqualTo(postComments.get(i).getAudit().getSecret().getSecret());
				}
			}
		}
