...
class DefaultJdbcParameterSourceConverterTest {
	...
	void convert() {
		...
		assertThat(uuid).isExactlyInstanceOf(byte[].class);
	...
	void constructorDuplicatedConverter() {
		List<Converter<?, ?>> converters = new ArrayList<>();
		converters.add(Java8TimeParameterTypeConverter.InstantParameterTypeConverter.INSTANCE);
		...
		assertThatThrownBy(() -> new DefaultJdbcParameterSourceConverter(converters))
			...
			.hasMessageContaining(Java8TimeParameterTypeConverter.InstantParameterTypeConverter.class.getName())
