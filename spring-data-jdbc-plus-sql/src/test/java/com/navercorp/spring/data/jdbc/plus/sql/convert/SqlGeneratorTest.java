...
/**
 * COPY {@link org.springframework.data.relational.core.convert.SqlGeneratorUnitTests}
 */
...
class SqlGeneratorTest {
	...
	public void joinForSimpleReference() {
		...
		SoftAssertions.assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinTable().getName()).isEqualTo(quoted("REFERENCED_ENTITY"));
			softly.assertThat(join.getJoinColumn().getTable()).isEqualTo(join.getJoinTable());
			softly.assertThat(join.getJoinColumn().getName()).isEqualTo(quoted("DUMMY_ENTITY"));
			softly.assertThat(join.getParentId().getName()).isEqualTo(quoted("id1"));
			softly.assertThat(join.getParentId().getTable().getName()).isEqualTo(quoted("DUMMY_ENTITY"));
		});
	}
	...
	public void joinForSecondLevelReference() {
		...
		SoftAssertions.assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinColumn().getTable()).isEqualTo(join.getJoinTable());
			softly.assertThat(join.getJoinColumn().getName()).isEqualTo(quoted("REFERENCED_ENTITY"));
			softly.assertThat(join.getParentId().getName()).isEqualTo(quoted("X_L1ID"));
			softly.assertThat(join.getParentId().getTable().getName()).isEqualTo(quoted("REFERENCED_ENTITY"));
		});
	}
	...
	public void joinForOneToOneWithoutId() {
		...
		SoftAssertions.assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinColumn().getName()).isEqualTo(quoted("PARENT_OF_NO_ID_CHILD"));
